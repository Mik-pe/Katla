...
fn test_abs_diff() {
    ...
    assert_eq!(abs_diff(-3.0, 5.0), 8.0);
}
...
#[test]
fn test_fract() {
    assert!((fract(3.7) - 0.7).abs() < 0.001);
    assert!((fract(-3.7) - 0.7).abs() < 0.001);
}
...
fn test_saturating_add() {
    ...
    // Large addition that doesn't overflow (2e38 < f32::MAX)
...
fn test_saturating_sub() {
    ...
    // Large subtraction that doesn't overflow
