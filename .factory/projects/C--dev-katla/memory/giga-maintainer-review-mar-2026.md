...
**Analysis:** The tonemap pass needs `objects[0]` populated with tonemap params. This is currently done in the graph before frame execution.

**Question:** Should the graph know about storage buffer layout? Or should the fullscreen pass execution callback handle this?
...
- Transient texture management could be a separate module
...
5. **Consider Extracting Transient Texture Module** (optional)
   - If `graph.rs` continues to grow, extract `TransientTexture` and related methods to `transient.rs`
   - Only if the file becomes genuinely unmanageable (>1000 lines)
