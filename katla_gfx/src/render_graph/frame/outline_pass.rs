...
impl<'a> Frame<'a> {
    ...
    ///
    /// Two sub-passes within a single render pass:
    /// 1. Stencil mark: render selected objects to stencil buffer (ref=1)
    /// 2. Outline draw: render inverted-culled shell where stencil != 1
    ///
    /// The outline is written to the HDR color buffer so it gets tonemapped naturally.
    pub(super) fn execute_outline_pass(
        ...
        // Depth attachment (load from depth prepass, don't clear)
        let depth_view = self
            .renderer
            .frame_context
        ...
        // Stencil attachment (same image as depth, stencil aspect)
        ...
        // Begin rendering with depth AND stencil
        cmd.begin_rendering(
            &[color_attachment],
        ...
        // === Sub-pass 2: Outline Draw ===
        self.execute_outline_draw(cmd, &data)?;

        // === Sub-pass 3: Wallhack Overlay ===
        self.execute_overlay(cmd, &data)?;
    ...
    /// Render a semi-transparent overlay where stencil == 1 (wallhack effect).
    /// Shows selected object through other geometry using alpha blending.
    fn execute_overlay(
        &mut self,
        cmd: &CommandBuffer,
        data: &PassExecutionData,
        ...
        let (overlay_pipeline, overlay_layout) = self
            .renderer
            .outline
            .overlay_pipeline
            .and_then(|h| self.renderer.asset_registry.get_pipeline_vk_handles(h))
        ...
        let storage_ds = self.renderer.storage_descriptor_sets[frame_idx].vk_set();
        cmd.bind_descriptor_sets(overlay_layout, 0, &[storage_ds], &[]);
