...
mod tests {
    ...
    #[test]
    fn test_default_gui_state() {
        let state = GuiState::default();
        assert_eq!(state.left_panel_width, 220.0);
        assert_eq!(state.right_panel_width, 280.0);
        assert_eq!(state.asset_browser_height, 200.0);
    }
...
asset_browser_height = 250
"#;
        let state = GuiState::parse_toml(content);
        assert_eq!(state.left_panel_width, 300.0);
        assert_eq!(state.right_panel_width, 350.0);
        assert_eq!(state.asset_browser_height, 250.0);
    }
    ...
    fn test_to_toml() {
        let state = GuiState {
            ...
            asset_browser_height: 180.0,
        };
        let toml = state.to_toml();
        assert!(toml.contains("left_panel_width = 250"));
        assert!(toml.contains("right_panel_width = 300"));
        assert!(toml.contains("asset_browser_height = 180"));
    }
}
