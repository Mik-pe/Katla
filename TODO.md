...
- [ ] **SceneSnapshot doesn't preserve physics components** — `SceneSnapshot::spawn_from_descriptor()` in `katla_app/src/application/game_state.rs` (lines ~125-300) only applies `transform`, `drawable`, `script`, `EntitySource`, and `NameComponent`. It never reads or applies `rigid_body`, `collider_shape`, `physics_material`, `trigger_volume`, or `collision_filter` from the descriptor. **Impact**: clicking Play then Stop in the editor strips all physics bodies from the restored scene. PhysicsActive(true) at builder init means the user sees a "frozen" post-settling scene after stop instead of returning to the pre-physics state. **Fix**: mirror the load-path logic from `SceneManager::load_scene` in `spawn_from_descriptor` — add `RigidBodyComponent`, `ColliderShapeComponent`, `PhysicsMaterialComponent`, `TriggerVolumeComponent`, `CollisionFilterComponent` to the spawned entity based on the descriptor fields.
...
- [ ] **Default scene path inconsistency between test sync and runtime** — `test_regenerate_default_scene` writes the canonical RON to `DEFAULT_SCENE_PATH` (`"assets/scenes/default.katla"`) relative to the test's cwd. When run via `cargo test -p katla_app`, cwd is `katla_app/`, so it writes to `katla_app/assets/scenes/default.katla`. The runtime resolves the same path against the **workspace root** (`/Users/mikpe/dev/Katla/assets/scenes/default.katla`), which is the file the editor actually loads. The two files can drift silently. **Impact**: easy to forget the second `cp` after running the regenerate test, leaving the editor loading stale data. **Fix**: pick one canonical location and have both test and runtime resolve against it (e.g. via `CARGO_MANIFEST_DIR` or a shared `paths::assets_dir()` helper).
