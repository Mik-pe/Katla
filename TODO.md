...
- **Sub-tasks:**
  - [ ] 161a. Replace `Rc<RefCell<Camera>>` with direct ownership — Camera is trivially simple (one `EntityId` field, all data fetched from ECS on demand), no clones of the Rc, no threading. The `Rc<RefCell<>>` is pure overhead. — (medium, low risk)
    - [ ] 161a1. Change field type from `Rc<RefCell<Camera>>` to `Camera` in `mod.rs` and `builder.rs` — (small, low risk)
    - [ ] 161a2. Replace 19 `.borrow()` reads with direct field access — gizmo.rs (8), renderer.rs (4), init.rs (1), editor/agent.rs (2), editor/mod.rs (2), serialization.rs (1), editor/agent.rs (1 borrow_mut) — (small, low risk)
    - [ ] 161a3. Replace 2 `.borrow_mut()` writes with direct mutation — serialization.rs (1), init.rs (1) — (small, low risk)
    - [ ] 161a4. Verify with `cargo check` and `cargo test` — (small, low risk)
...
- **Sub-tasks:**
  ...
  - [ ] 164c. Replace `if pass.name == "geometry"` with `PassKind::Geometry` dispatch — in `frame/mod.rs:363`, the `PassKind` enum already exists and `GeometryPass` sets `kind: Some(PassKind::Geometry)`. Independent of #166 — if #166 later makes particles a proper pass, this check moves into that pass's execution. — (small, low risk)
