...
- **Render-graph liveness and execution-plan completion** — Metal now consumes exact compiled pass records with pass-local submissions. Explicit exported resources, side-effect roots, dead-pass culling, and culling-aware diagnostics are being completed for #34/#56 before graph-declared attachments and generic executable handlers.
...
- Metal consumes the compiler's canonical ordered pass records directly. Every record retains exact `PassId`, declaration index, name, and executable semantic; submissions are pass-local, repeated semantic categories are valid, and unsupported handlers fail before command-buffer creation.
- Pass liveness is explicit policy. Presented/read-back resources are exported roots, non-resource work is marked as a side effect, and only true data producers are retained. `LoadOp::Load` and blending passes declare read-before-write dependencies so culling cannot discard preserved target contents.
...
- Metal object-buffer overflow is a hard renderer error; invalid offsets are never submitted after a warning-and-continue fallback. Completed command buffers are also checked for terminal GPU failure and return structured backend/status/NSError details.
