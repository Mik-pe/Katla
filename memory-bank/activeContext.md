...
- **PhysicsActive(true) at builder init** — both headless and windowed now default to active, so the scene plays its falling demo on load. User can still pause via play/pause toggle. SceneSnapshot physics preservation fixed — `spawn_from_descriptor()` now restores all physics components (RigidBody, ColliderShape, PhysicsMaterial, TriggerVolume, CollisionFilter).
