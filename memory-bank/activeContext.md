...
- Metal bindless argument buffers are initialized lazily from a compiled fragment function. Tier 2 uses direct `MTLResourceID` entries, supported Tier 1 devices use shader-reflected layout, and unsupported Apple paravirtual Tier 1 devices fail before an invalid Objective-C selector or GPU submission.
- Objective-C calls that may raise are scoped behind exception conversion and produce typed renderer errors; foreign exceptions must never unwind through Rust.
...
- Metal application initialization failures return a non-zero process status; unsupported GPU environments cannot look like successful headless renders.
