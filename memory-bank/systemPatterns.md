...
The compiled dependency DAG and execution order are authoritative. Backends consume compiled pass identity and access metadata rather than maintaining a second name-based graph. Metal executes the compiler's ordered record stream directly, preserving exact `PassId` and pass-local submissions; repeated semantic categories and arbitrary dependency-valid order are normal graph shapes. Any unsupported executable kind must fail while compiling the backend plan, before command-buffer creation.
