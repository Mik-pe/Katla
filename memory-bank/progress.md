...
- Added permanent CI with an explicit two-generation macOS matrix: `macos-26` is the primary Apple Silicon Metal job and `macos-15` is the compatibility job. Removed mutable/deprecated runner choices from the policy, documented the upgrade process, and added Linux workspace-wide formatting, check, test, and Clippy coverage.
