...
- Made Metal frame topology optional and fail-fast (#49): empty, UI-only, geometry-only, geometry+UI, and scene-without-UI schedules are accepted; absent semantic passes no longer trigger hidden depth, geometry, tonemap, or fence work; fixed-encoder limitations are named as such; and terminal command-buffer failures propagate as structured renderer errors.
