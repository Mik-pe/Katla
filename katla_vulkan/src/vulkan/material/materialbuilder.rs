...
impl MaterialBuilder {
    ...
    /// Build the material pipeline for full PBR with storage buffers.
    ///
    /// This creates a two-set layout:
    /// - Set 0: Storage buffers for frame_data (binding 0) and objects array (binding 1)
    /// - Set 1: 10 texture bindings (5 textures + 5 samplers for full PBR)
    ...
    ///   - Binding 8-9: Emission + sampler
    ///
    /// Use this for PBR materials that need all texture maps.
    pub fn build_with_storage_pbr(self) -> Result<MaterialPipeline, MaterialBuildError> {
        let vertex_binding = self
            .vertex_binding
            .ok_or(MaterialBuildError::MissingVertexBinding)?;
        ...
        // Set 1: Textures (separate image + sampler for WGSL)
        // Full PBR layout: 5 textures + 5 samplers = 10 bindings
        let texture_set_layout = DescriptorLayoutBuilder::new()
            // Albedo
            .add_binding(0, vk::DescriptorType::SAMPLED_IMAGE, vk::ShaderStageFlags::FRAGMENT, 1)
            .add_binding(1, vk::DescriptorType::SAMPLER, vk::ShaderStageFlags::FRAGMENT, 1)
            // Normal
            .add_binding(2, vk::DescriptorType::SAMPLED_IMAGE, vk::ShaderStageFlags::FRAGMENT, 1)
            .add_binding(3, vk::DescriptorType::SAMPLER, vk::ShaderStageFlags::FRAGMENT, 1)
            // Metallic/Roughness
            .add_binding(4, vk::DescriptorType::SAMPLED_IMAGE, vk::ShaderStageFlags::FRAGMENT, 1)
            .add_binding(5, vk::DescriptorType::SAMPLER, vk::ShaderStageFlags::FRAGMENT, 1)
            // Occlusion
            .add_binding(6, vk::DescriptorType::SAMPLED_IMAGE, vk::ShaderStageFlags::FRAGMENT, 1)
            .add_binding(7, vk::DescriptorType::SAMPLER, vk::ShaderStageFlags::FRAGMENT, 1)
            // Emission
            .add_binding(8, vk::DescriptorType::SAMPLED_IMAGE, vk::ShaderStageFlags::FRAGMENT, 1)
            .add_binding(9, vk::DescriptorType::SAMPLER, vk::ShaderStageFlags::FRAGMENT, 1)
            .build(&self.context.device)
            .map_err(|e| MaterialBuildError::DescriptorLayoutFailed(format!("{:?}", e)))?;
        ...
        // Create MaterialPipeline with storage buffer configuration
        let material_pipeline = MaterialPipeline::new_storage(
            pipeline,
            uniform_set_layout,
            texture_set_layout,
            self.context.clone(),
    ...
    /// Build the material pipeline for bindless texture rendering.
    ///
    /// This creates a two-set layout:
    /// - Set 0: Storage buffers for frame_data (binding 0) and objects array (binding 1)
    /// - Set 1: Bindless texture array + shared sampler (from BindlessTextureManager)
    ///
    /// Texture indices are passed via ObjectUniforms.texture_indices.
    /// The bindless layout must be obtained from BindlessTextureManager::descriptor_layout().
    ///
    /// # Arguments
    /// * `bindless_layout` - The descriptor set layout from BindlessTextureManager
    ///
    /// # Example
    /// ```ignore
    /// let bindless_manager = BindlessTextureManager::new(context.clone())?;
    /// let material = MaterialBuilder::new(context.clone())
    ///     .with_vertex_shader(...)
    ///     .with_fragment_shader(...)
    ///     .build_bindless(bindless_manager.descriptor_layout().vk())?;
    /// ```
    pub fn build_bindless(
        ...
        // Set 1: Bindless textures (layout provided by BindlessTextureManager)
        // We don't own this layout, so we clone the wrapper for reference counting

        let color_format = self.color_format;
        let depth_format = self.depth_format;

        // Build pipeline with bindless texture layout
        let mut pipeline_builder = PipelineBuilder::new(self.context.clone())
            .with_shaders(vert_shader.module, frag_shader.module)
            .with_entry_points(
                vert_shader.entry_point.clone(),
            ...
            .with_depth_test(self.depth_test, self.depth_write, self.depth_compare_op)
            .with_descriptor_layouts(vec![uniform_set_layout, bindless_layout]);
        ...
        // Create MaterialPipeline for bindless rendering
        // Note: We don't store the bindless layout since it's owned by BindlessTextureManager
        let material_pipeline = MaterialPipeline::new_bindless(
            pipeline,
            uniform_set_layout,
            self.context.clone(),
        );
