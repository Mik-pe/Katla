...
fn main() {
    let mut application = application::ApplicationBuilder::new()
    ...
    // //TODO:
    // // Add a "nicer" interface to add models(future lights, whatever,) to the scene.
    // // Move the current threading into the renderer, so if we ever use it,
    // // it stays hidden inside the renderer. The "Scene" should not care about it
    ...
    //     let mut model_cache = util::FileCache::<GLTFModel>::new();
    //     {
    ...
    //         finished_tx
    //             .send(FinishedUpload::Model(Mesh::new_from_cache(
    //                 model_cache.read(PathBuf::from("resources/models/Avocado.glb")),
    //                 context.clone(),
    //                 render_pass,
    //                 num_images,
    //                 Vec3::new(-1.0, 0.0, 0.0),
