//! Six-component query iterators.

use super::QueryData;
use crate::{Component, ComponentStorage, ComponentStorageManager, EntityId};
use std::any::TypeId;

/// Iterator for querying six components (all immutable).
pub struct QueryIter6<
    'a,
    T1: Component,
    T2: Component,
    T3: Component,
...
/// Iterator for querying six components (first mutable, rest immutable).
pub struct QueryIter6MutRefRefRefRefRef<
    'a,
    T1: Component,
    T2: Component,
    T3: Component,
...
> QueryData for (&T1, &T2, &T3, &T4, &T5, &T6)
{
    ...
    fn fetch(storage: &mut ComponentStorageManager) -> Self::Iter<'_> {
        ...
        if let (Some(s1), Some(s2), Some(s3), Some(s4), Some(s5), Some(s6)) =
